...
import { getDBConnection, saveTodo } from '../db/Database.js';

export default function DB_AddTodo({ navigation, route }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSaving(true); //for loading spinner
      const db = await getDBConnection(); //Open DB
      await saveTodo(db, newTodo); //Insert into DB
      console.log('Todo saved to DB:', newTodo.title);
      navigation.goBack();
    } catch (err) {
